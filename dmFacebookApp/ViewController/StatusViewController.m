...
//  dmFacebookApp
//
//  Created by Saquib Waheed on 12/29/12.
//
//
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text=[[self.statusList objectAtIndex:indexPath.row]objectForKey:@"message"];
...
- (void) runFQL
{
    ...
    if ([[FBSession activeSession] isOpen]) {
        NSString* query=@"SELECT  message from status where uid=me()";
        ...
        [FBRequestConnection startWithGraphPath:@"/fql" parameters:param HTTPMethod:@"GET" completionHandler:^(FBRequestConnection* connection,
            ...
            if (error) {
                ...
                NSLog(@"Result: %@", result);
                statusList=[result objectForKey:@"data"];
            }
        }];
    }
}
